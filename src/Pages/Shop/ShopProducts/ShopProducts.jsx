...
const ShopProducts = () => {
    ...
    // Initialize displayItems and filteredItems when items are fetched
    useEffect(() => {
        if (items && items.length > 0) {
            const sortedAlphabetically = [...items].sort((a, b) => a.name.localeCompare(b.name));
            setFilteredItems(sortedAlphabetically); // Set filtered items to all items initially
            setDisplayItems(sortedAlphabetically); // Display all items initially
        }
    }, [items]);
    ...
    const handleSortChange = (sortOption) => {
        ...
        switch (sortOption) {
            case 'newArrival':
                sorted = sorted.filter((item) => item.section === 'newArrival');
                break;
            case 'featured':
                sorted = sorted.filter((item) => item.section === 'featured');
                break;
            case 'onSale':
                sorted = sorted.filter((item) => item.section === 'onSale');
                break;
            case 'a-z':
    ...
    const handleLoadMore = () => {
        ...
        setTimeout(() => {
            ...
            setVisibleProducts((prev) => Math.min(prev + 20, displayItems.length)); // Load 4 more products each time
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {isLoading ? (
                ...
                <div className={`grid gap-1 md:gap-2 w-full ${columns === 1 ? 'grid-cols-1' : columns === 2 ? 'grid-cols-2' : columns === 3 ? 'grid-cols-3' : columns === 4 ? 'grid-cols-4' : 'grid-cols-5'}`}>
                    {displayItems.slice(0, visibleProducts).map((product) => (
            ...
            <div className="text-center mt-2">
                ...
                <Button
                    ...
                    disabled={visibleProducts >= displayItems.length}
                >
